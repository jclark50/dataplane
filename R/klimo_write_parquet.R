...
# =============================================================================
# This file defines:
#   - klimo_write_parquet(): main writer (hybrid validate/convert + audit metadata)
#   - klimo_spec_preview(): inspect how a spec matches a dataset (units + scaling plan)
#   - klimo_spec_default(): build a default spec tailored to the columns present
#   - klimo_spec_set_units(): easy post-hoc edits to a spec’s units
#   - klimo_spec_set_scale(): easy post-hoc edits to a spec’s scale
#
# S3 objects:
...
#'
#' `klimo_spec_default()` takes a dataset (data.frame/data.table) and returns a `klimo_spec`
...
#' @export
klimo_spec_default <- function(x, system = c("metric", "imperial"), include_unmatched = TRUE) {
  .klimo_require("data.table")
...
#' @export
klimo_spec_set_units <- function(spec, columns, metric_units = NULL, imperial_units = NULL) {
  ...
  if (!any(dt$.hit)) {
    .klimo_warn("klimo_spec_set_units(): no columns matched. Provided: %s", paste(columns, collapse = ", "))
  }
...
#' @export
klimo_spec_set_scale <- function(spec, columns, suggested_scale) {
  if (!inherits(spec, "klimo_spec")) .klimo_stop("spec must be a klimo_spec.")
  .klimo_require("data.table")
  ...
  if (!any(dt$.hit)) {
    .klimo_warn(
      "klimo_spec_set_scale(): no columns matched. Provided: %s",
...
#   
#   if (!any(dt$.hit)) {
#     .klimo_warn("klimo_spec_set_scale(): no columns matched. Provided: %s", paste(columns, collapse = ", "))
#   }
#   
...
.klimo_normalize_field_spec <- function(x, field_spec, declared_system = c("metric", "imperial")) {
  ...
  if (.klimo_is_scalar_chr(field_spec)) {
    if (field_spec %in% c("metric", "imperial")) {
      ...
      spec_dt <- klimo_spec_default(x, system = field_spec, include_unmatched = TRUE)$spec_dt
...
#             collapse = "; ")
#     )
#     if (isTRUE(strict)) .klimo_stop("klimo_write_parquet(): %s", msg) else .klimo_warn("klimo_write_parquet(): %s", msg)
#   }
#   
#   if (nrow(miss)) {
#     .klimo_warn(
#       "klimo_write_parquet(): %d columns have declared units but no observed attr(unit). (This is ok if you are only documenting.)",
#       nrow(miss)
#     )
#   }
#   
...
.klimo_unit_validate <- function(preview_dt, observed_dt, strict = FALSE) {
  ...
  if (nrow(miss)) {
    ...
    msg <- sprintf(
      "klimo_write_parquet(): %d columns have declared units but no observed attr(unit). (Ok for documentation-only.)",
      nrow(miss)
    )
...
# =============================================================================
# Writer: klimo_write_parquet (hybrid Option 1 + 2 + audit Option 5)
# =============================================================================
...
klimo_write_parquet <- function(
  ...
  if (is.null(spec)) {
    ...
    .klimo_msg("klimo_write_parquet(): using field_spec (declared_system=%s)", spec$declared_system)
  }
  
  # Preview with resolved declared units and scaling/encoding plan
  prev <- klimo_spec_preview(
    x,
    field_spec = spec,
    ...
    if (nrow(decl)) {
      ...
      if (nrow(miss)) {
        ...
        if (isTRUE(tag_missing_units_warn)) {
          ...
          .klimo_warn(
            "klimo_write_parquet(): tag_missing_units=TRUE. Tagged %d column(s) missing attr(unit) using declared units (%s). Example(s): %s",
            nrow(miss),
  ...
  kv <- .klimo_metadata_kv(spec, prev, audit, extra = list(
    "klimo:writer" = "klimo_write_parquet",
...
# klimo_write_parquet(dat, "out.parquet", field_spec="metric", unit_mode="none")
# 
# B) Validate (warn if mismatched; allow missing unit tags)
# klimo_write_parquet(dat, "out.parquet", field_spec="imperial", unit_mode="validate")
# 
# C) Validate strictly (stop on mismatches)
# klimo_write_parquet(dat, "out.parquet", field_spec="imperial", unit_mode="validate", unit_strict=TRUE)
# 
...
# spec_m <- klimo_spec_default(dat, system = "metric")
# prev <- klimo_spec_preview(dat, field_spec = spec_m, scaled = FALSE)
# print(prev)
# 
# # ---- modify one column’s units after-the-fact ------------------------------
# # e.g., your solar column is actually in W/m^2 already (default), but if you needed:
# 
# spec_m2 <- klimo_spec_set_units(spec_m, "wind", metric_units = "kt", imperial_units = "kt")
# # class(spec_m2)
# # ---- write with documentation-only (no conversion, no scaling) -------------
# klimo_write_parquet(dat, "out_unscaled_doc.parquet", field_spec = spec_m2,
#                    unit_mode = "validate", scaled = FALSE)
# 
# # ---- write with scaling (still no unit conversion) -------------------------
# klimo_write_parquet(dat, "out_scaled_doc.parquet", field_spec = spec_m2,
...
# # unit(dat$ta) <- "degF"
# # unit(dat$td) <- "degF"
# # klimo_write_parquet(dat, "out_converted.parquet", field_spec = "metric",
# #                    unit_mode = "convert", scaled = FALSE)
# 
# # If columns are untagged but you know source units:
# # klimo_write_parquet(dat, "out_converted2.parquet", field_spec = "metric",
# #                    unit_mode = "convert",
# #                    src_units = c(ta = "degF", td = "degF"),
# #                    scaled = FALSE)
# 
...
# 
# spec_m <- klimo_spec_default(dat, system = "metric")
# spec_i <- klimo_spec_default(dat, system = "imperial")
# 
# 
# prev_unscaled <- klimo_spec_preview(dat, field_spec = spec_m, scaled = FALSE)
# prev_unscaled
# 
# 
# spec_m2$spec_dt
...
# 
# 
# spec_m2 <- klimo_spec_set_units(spec_m, "solar", metric_units = "m/s", imperial_units = "kW/m^2")
# klimo_spec_preview(dat, field_spec = spec_m2, scaled = TRUE)
# 
# 
# 
# spec_m3 <- klimo_spec_set_scale(spec_m2, c("lat", "lon"), suggested_scale = 5)
# klimo_spec_preview(dat, field_spec = spec_m3, scaled = TRUE)
# 
# 
# 
# unit(dat$ta) <- "degC"
...
# 
# 
# klimo_write_parquet(dat, 'test.parquet', unit_mode="validate", field_spec="metric")
# 
# ?klimo_write_parquet
# 
# 
...
# #
# #
# # rm(klimo_spec_default)
# # rm(klimo_spec_set_units)
# # rm(klimo_spec_set_scale)
# # rm(klimo_spec_preview)
# # rm(klimo_write_parquet)
# # # =============================================================================
# #
# #
# # dat
